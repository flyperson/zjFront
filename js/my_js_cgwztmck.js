...
mui.plusReady(function(){
	...
	xiugai.addEventListener('tap',function(){
		...
		window.addEventListener('json',function(event){
			...
			//alert(xzwztm1+"   "+hehe1);
			xzwztm=xzwztm1.split("||");
			hehe=hehe1.split("||"); 
			for(var j=wztmnr.length-1,i=0;i<xzwztm.length-1;i++,j++){
				id=id+1;
				mywztm(id,xzwztm[i],j,hehe[i]);
   			}
			function mywztm(id,wztm,j,hehe){
				var id=id;
				var hehe=hehe;
				var wztm=wztm;  
				var num=j; 
				...
				var ul = document.createElement("ul");
				ul.className = "mui-table-view";
				ul.innerHTML = '<li id="li'+num+'" class="mui-table-view-cell"><img id="t'+num+'" class="mui-media-object mui-pull-right" src="../images/wztmzj.jpg" style="width:50px;height:50px;"><div class="mui-media-body">'+wztm+'</div><p>部位：'+hehe+'</p></li>'
				wztmck.appendChild(ul);
			};
		})
...
//		alert("您已点击整合,请等待");
		...
		for(var k=0;k<imgArrayFirst.length;k++){
			...
			//alert(srcTrue);
			if(srcTrueFirst=="../images/add.png"){
				//这个路径也要改，IP为服务器的IP
//				srcTrueFirst="http://202.105.179.167:8080/hxajxtapp/hxajxt/upload/add.jpg";
				srcTrueFirst="http://192.168.0.14:8081/hxajxtapp/hxajxt/upload/add.jpg";

//				srcTrueFirst="http://192.168.0.14:8081/hxajxtapp/images/add.jpg";
			}
		...
		for(var i=0;i<imgArray.length;i++){
			...
			//alert(srcTrue);
			if(srcTrue=="../images/add.png"){
				//这个路径也要改，IP为服务器的IP
				srcTrue="http://202.105.179.167:8080/hxajxtapp/hxajxt/upload/add.jpg";
			}
